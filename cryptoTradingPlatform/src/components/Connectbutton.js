...
function Connectbutton({tokenInfo}) {
    ...
    const handleSignMessage = async ({
        ...
        try {
            ...
            const signature = await web3.eth.personal.sign(
                `Signing for connecting wallet: ${nonce}`,
                publicAddress,
    ...
    const handleSignMessageForWC = async ({
        publicAddress,
        nonce,
    }) => {
        try {
            const signature = await connector.signPersonalMessage([`Signing for connecting wallet: ${nonce}`, publicAddress])
            return { publicAddress, signature };
        } catch (err) {
            throw ("new Error({ code : 403 })");
        }
    ...
    useEffect(() => {
        if (tokenInfo !== undefined && tokenInfo.addr !== undefined && tokenInfo.name !== undefined && walletAddress === "Connect") {
            handleShow();
        }
    }, [tokenInfo])
        ...
        window.ethereum.request({
            ...
            try {
                ...
                .then(coinbase => {
                    ...
                    fetch(SERVER_URL + "/users?publicAddress=" + publicAddress)
                        .then((response) => response.json())
                        .then((users) =>
                        ...
                        .then(res => {
                            setWalletAddress(accounts[0])
                            reactLocalStorage.set('login', 'connected');
                            reactLocalStorage.set('apiKey', res.accessToken);
                        })
                })
            } catch {
    ...
    useEffect(() => {
        ...
        function handleAccountsChanged(accounts) {
            ...
            if (accounts[0]) {
                reactLocalStorage.remove('login')
                setWalletAddress("Connect");
                toastMessage("Please sign for connecting wallet", 3500)
                web3.eth.getCoinbase()
                    .then(coinbase => {
                        const publicAddress = coinbase.toLowerCase();
        ...
        <div>
            ...
            <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
                ...
                <Modal.Body>
                    ...
                    {(walletAddress !== "Connect") && <div className="logout" onClick={disconnectWallet}>
